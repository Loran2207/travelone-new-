...
// ---- MY TRIPS tab (one save mechanism — saving a trip adds it here) ----
export function MyTripsScreen({ allTrips, myTrips, savedTrips, filter, sort, onFilter, onOpenGuide, onHeart, onExplore, onMap }: {
  ...
  return (
    <div className="screen screen-pad">
      ...
      {mine.length === 0 ? (
        <div className="emptystate">
          <div className="es-title">No trips yet</div>
          <div className="es-sub">Tap <iconify-icon icon="solar:heart-linear"></iconify-icon> on any guide to save it here, or build one from a list on the map</div>
          <button className="es-cta" onClick={onExplore}><iconify-icon icon="solar:magnifer-linear"></iconify-icon> Explore guides</button>
          <button className="es-cta2" onClick={onMap}><iconify-icon icon="solar:map-point-bold"></iconify-icon> Find places on the map</button>
        </div>
      ) : (
